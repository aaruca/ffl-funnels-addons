...
class WooBooster_Rule_Form
{
    ...
    public function render()
    {
        ...
        $title = $is_edit
            ? __('Edit Rule', 'ffl-funnels-addons')
            : __('Add New Rule', 'ffl-funnels-addons');
        ...
        echo esc_html__('Back to Rules', 'ffl-funnels-addons');
        echo '</a>';
        echo '</div>';
        ...
        if (isset($_GET['saved']) && '1' === $_GET['saved']) {
            ...
            echo '<span>' . esc_html__('Rule saved successfully.', 'ffl-funnels-addons') . '</span>';
            echo '</div>';
        }
        ...
        echo '<label class="wb-field__label" for="wb-rule-name">' . esc_html__('Rule Name', 'ffl-funnels-addons') . '</label>';
        ...
        // Priority.
        echo '<div class="wb-field">';
        echo '<label class="wb-field__label" for="wb-rule-priority">' . esc_html__('Priority', 'ffl-funnels-addons') . '</label>';
        echo '<div class="wb-field__control">';
        ...
        echo '<p class="wb-field__desc">' . esc_html__('Lower number = higher priority. If multiple rules match, the lowest priority wins.', 'ffl-funnels-addons') . '</p>';
        echo '</div></div>';
        ...
        echo '<p class="wb-section-desc">' . esc_html__('Groups are combined with OR. Conditions within a group are combined with AND. Use “Entire store” when the rule should apply to every product (no category, tag, or search value needed).', 'ffl-funnels-addons') . '</p>';
        ...
        foreach ($condition_groups as $group_id => $conditions) {
            ...
            if ($group_index > 0) {
                echo '<button type="button" class="wb-btn wb-btn--danger wb-btn--xs wb-remove-group" title="' . esc_attr__('Remove Group', 'ffl-funnels-addons') . '">&times;</button>';
            }
            echo '</div>';
            ...
            foreach ($conditions as $cond) {
                ...
                echo '<span class="wb-condition-store-all-hint">' . esc_html__('Applies to every product. Use exclusions below if you need to narrow it.', 'ffl-funnels-addons') . '</span>';
                ...
                // Min quantity.
                echo '<input type="number" name="' . esc_attr($field_prefix . '[min_quantity]') . '" value="' . esc_attr($c_min_qty) . '" min="1" class="wb-input wb-input--sm wb-input--w60" title="' . esc_attr__('Min cart qty (coupon rules only)', 'ffl-funnels-addons') . '" placeholder="Qty">';
                ...
                echo ($cex_has ? '&#9660;' : '&#9654;') . ' ' . esc_html__('Condition Exclusions', 'ffl-funnels-addons');
                echo '</button>';
                ...
                echo '<input type="text" class="wb-input wb-cond-exclude-cats__input" placeholder="' . esc_attr__('Search categories…', 'ffl-funnels-addons') . '" autocomplete="off">';
                ...
                echo '<input type="text" class="wb-input wb-cond-exclude-prods__input" placeholder="' . esc_attr__('Search products…', 'ffl-funnels-addons') . '" autocomplete="off">';
                ...
                echo '<input type="number" name="' . esc_attr($field_prefix . '[exclude_price_min]') . '" value="' . esc_attr($cex_price_min) . '" class="wb-input wb-input--sm wb-input--w90" placeholder="' . esc_attr__('Min $', 'ffl-funnels-addons') . '" step="0.01" min="0">';
                echo '<span>&mdash;</span>';
                echo '<input type="number" name="' . esc_attr($field_prefix . '[exclude_price_max]') . '" value="' . esc_attr($cex_price_max) . '" class="wb-input wb-input--sm wb-input--w90" placeholder="' . esc_attr__('Max $', 'ffl-funnels-addons') . '" step="0.01" min="0">';
                echo '</div></div>';
        ...
        echo '<h3>' . esc_html__('Actions', 'ffl-funnels-addons') . '</h3>';
        echo '<p class="wb-section-desc">' . esc_html__('Groups are combined with OR. Actions within a group are combined with AND.', 'ffl-funnels-addons') . '</p>';
        ...
        foreach ($action_groups as $group_id => $actions) {
            ...
            echo '<span class="wb-action-group__label">' . esc_html__('Action Group', 'ffl-funnels-addons') . ' ' . ($a_group_index + 1) . '</span>';
            if ($a_group_index > 0) {
                echo '<button type="button" class="wb-btn wb-btn--danger wb-btn--xs wb-remove-action-group" title="' . esc_attr__('Remove Group', 'ffl-funnels-addons') . '">&times;</button>';
            }
            echo '</div>';
            ...
            foreach ($actions as $action) {
                ...
                echo '<select name="' . esc_attr($prefix . '[action_orderby]') . '" class="wb-select wb-select--inline" style="' . esc_attr($display_order_limit) . '" title="' . esc_attr__('Order By', 'ffl-funnels-addons') . '">';
                $orderbys = array(
                ...
                echo '<input type="number" name="' . esc_attr($prefix . '[action_limit]') . '" value="' . esc_attr($a_limit) . '" min="1" class="wb-input wb-input--sm wb-input--w70" style="' . esc_attr($display_order_limit) . '" title="' . esc_attr__('Limit', 'ffl-funnels-addons') . '">';
                ...
                echo '<p class="wb-field__desc wb-coupon-desc">' . esc_html__('Works with your existing WooCommerce coupons. Create coupons in Marketing > Coupons first.', 'ffl-funnels-addons') . '</p>';
                echo '<label class="wb-field__label">' . esc_html__('Select Coupon', 'ffl-funnels-addons') . '</label>';
                echo '<div class="wb-autocomplete wb-autocomplete--sm wb-coupon-search">';
                echo '<input type="text" class="wb-input wb-coupon-search__input" placeholder="' . esc_attr__('Search coupons…', 'ffl-funnels-addons') . '" value="' . esc_attr($cp_label) . '" autocomplete="off">';
                echo '<input type="hidden" name="' . esc_attr($prefix . '[action_coupon_id]') . '" class="wb-coupon-search__id" value="' . esc_attr($cp_id) . '">';
                echo '<div class="wb-autocomplete__dropdown"></div>';
                echo '</div>';
                echo '<div class="wb-field">';
                echo '<label class="wb-field__label">' . esc_html__('Custom Cart Message', 'ffl-funnels-addons') . '</label>';
                echo '<input type="text" name="' . esc_attr($prefix . '[action_coupon_message]') . '" class="wb-input wb-input--max-md" placeholder="' . esc_attr__('e.g. You got 15% off on Ammo products!', 'ffl-funnels-addons') . '" value="' . esc_attr($cp_message) . '">';
                echo '<p class="wb-field__desc">' . esc_html__('Leave empty for the default auto-apply message.', 'ffl-funnels-addons') . '</p>';
                echo '</div>';
                echo '</div>';
                ...
                echo ($has_exclusions ? '&#9660;' : '&#9654;') . ' ' . esc_html__('Action Exclusions', 'ffl-funnels-addons');
                echo '</button>';
                ...
                echo '<span class="wb-field__desc">' . esc_html__('Only include products in this price range', 'ffl-funnels-addons') . '</span>';
                echo '</div></div>';
            ...
            echo '+ ' . esc_html__('AND Action', 'ffl-funnels-addons');
            echo '</button>';
        ...
        echo $is_edit ? esc_html__('Update Rule', 'ffl-funnels-addons') : esc_html__('Create Rule', 'ffl-funnels-addons');
        echo '</button>';
