...
class Alg_Wishlist_Shortcodes
{
    ...
    public function render_aws_button($atts)
    {
        ...
        // Honour the configured wishlist icon; falls back to the filled heart.
        $icon_html = class_exists('\Alg_Wishlist_Core')
            ? \Alg_Wishlist_Core::default_icon_svg('alg-wishlist-icon')
            : '<svg class="alg-wishlist-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z"></path></svg>';
        ...
        <a href="#" class="<?php echo esc_attr($class); ?>" data-product-id="<?php echo esc_attr($id); ?>"
            ...
            <?php echo $icon_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- sanitised in Alg_Wishlist_Core. ?>
    ...
    public function render_count($atts)
    {
        ...
        if ($atts['icon'] === 'heart' || empty($atts['icon'])) {
            $icon_html = class_exists('\Alg_Wishlist_Core')
                ? \Alg_Wishlist_Core::default_icon_svg('alg-count-icon')
                : '<svg class="alg-count-icon" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>';

            // The icon_color attribute used to be an inline style on the SVG.
            // Wrap instead, so it applies to whichever icon is configured:
            // both `color` (via currentColor) and `stroke` inherit into the SVG.
            // Only wraps when the attribute is actually used, so the common
            // case emits identical markup to before.
            if ($icon_style !== '') {
                $icon_html = '<span class="alg-count-icon-color" style="' . esc_attr($icon_style) . '">' . $icon_html . '</span>';
            }
        }
