 ...
 *
 * Renders the Mapbox address autocomplete configuration panel
 * inside the FFLA admin shell.
 *
...
class FFL_Checkout_Admin
{
    ...
    public function save_settings(): void
    {
        ...
        $settings = [
            'mapbox_public_token'  => isset($_POST['mapbox_public_token'])
                ? sanitize_text_field(wp_unslash($_POST['mapbox_public_token']))
                : '',
    ...
    public function render_settings_page(): void
    {
        ...
        echo '<div class="wb-card__header"><h3>' . esc_html__('Mapbox Address Autocomplete', 'ffl-funnels-addons') . '</h3></div>';
        echo '<div class="wb-card__body">';
        echo '<p class="wb-section-desc">' . esc_html__('Configure Mapbox address autocomplete for WooCommerce checkout fields. When enabled, the billing and shipping address fields will show address suggestions as the customer types.', 'ffl-funnels-addons') . '</p>';
        ...
        FFLA_Admin::render_toggle_field(
            ...
            $enabled,
            __('Enable Mapbox address autocomplete on WooCommerce checkout billing and shipping address fields.', 'ffl-funnels-addons')
        );
        ...
        echo '<li>' . esc_html__('Create a free account at', 'ffl-funnels-addons') . ' <a href="https://account.mapbox.com/auth/signup/" target="_blank" rel="noopener">mapbox.com</a></li>';
        echo '<li>' . esc_html__('Go to Account → Access Tokens and copy your Default Public Token (or create a new one).', 'ffl-funnels-addons') . '</li>';
        echo '<li>' . esc_html__('Paste the token above and enable autocomplete.', 'ffl-funnels-addons') . '</li>';
